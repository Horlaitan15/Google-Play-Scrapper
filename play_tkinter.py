...
label_0 =Label(root,text="Google Play Scraper", width=20,font=("bold",30))
...
#this creates 'Label' widget for CSV File and uses place() method.
label_3 =Label(root,text="Enter CSV Output:", width=20,font=("bold",11))
...
def submit():
    ...
    termi.pack()
    cmd = f"scrapy crawl play -a parameters={{'url':{str(entry_1.get().split('&')).replace(' ', '')},'key':'{entry_2.get()}'}} -t csv -o {entry_3.get()}"
    process = subprocess.Popen(cmd.replace('&', '\&'), 
                    shell=True, stdout=subprocess.PIPE, 
                    bufsize=1, universal_newlines=True)
    
    def stop_run():
        process.kill()
        Button(root, text='Start Extracting' ,command=submit, width=20,bg="green",fg='white').place(x=180,y=280)

    Button(root, text='Stop Extracting' ,command=stop_run, width=20,bg="red",fg='white').place(x=180,y=280)
